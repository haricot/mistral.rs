...
| DSpark | LFM2.5 | Native Safetensors DSpark checkpoint (`--mtp-model`) | Supported with paged attention, static full-block verification |
...
LFM2.5 targets can attach Liquid AI's native Safetensors DSpark sidecar. mistral.rs detects
`Lfm2DSparkDraftModel` from `config.json`, captures the configured LFM target layers, runs the
five-layer attention drafter with interleaved RoPE, and applies the checkpoint's rank-256 Markov
head sequentially across the proposed block. The target's attention KV and ShortConv state are
committed transactionally after verification, so a rejected suffix cannot leak recurrent state.

```bash
mistralrs run -m LiquidAI/LFM2.5-2.6B \
  --mtp-model LiquidAI/LFM2.5-2.6B-DSpark \
  --mtp-n-predict 8 \
  --mtp-draft-sampling greedy
```

The target and drafter are loaded from their original Safetensors checkpoints; GGUF conversion is
not involved. Omitting `--mtp-n-predict` uses the checkpoint's full `block_size - 1` draft width.
The confidence head is loaded and evaluated, while the current homogeneous-batch engine uses
DSpark's exact static full-block verification mode. Compact confidence-scheduled ragged
verification is not enabled by this mode.
