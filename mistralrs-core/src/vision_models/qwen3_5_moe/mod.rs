...
impl Qwen3_5MoeModel {
    pub fn new(
        ...
        let vision_topology = normal_loading_metadata
            .topology
            .as_ref()
            .and_then(|topology| topology.submodels.vision.clone());
        let load_vision = vision_topology
            .as_ref()
            .map(|vision| vision.load)
            .unwrap_or(true);
        ...
        let vision = if load_vision {
            ...
            let vision_device = vision_topology
                .as_ref()
                .and_then(|vision| vision.device.clone())
                .unwrap_or_else(|| normal_loading_metadata.real_device.clone());
            Some(Qwen3VLVisionModel::new(
                &cfg.vision_config,
                vision_vb.set_device(vision_device),
            )?)
        } else {
            info!("Qwen3.5 MoE topology disabled the vision submodel; loading text path only.");
            None
        };
    ...
    pub fn forward(
        ...
        if let Some(pixel_values) = &pixel_values {
            let Some(vision) = self.vision.as_ref() else {
                candle_core::bail!(
                    "Vision inputs were provided, but the vision submodel is disabled by topology."
                );
            };
