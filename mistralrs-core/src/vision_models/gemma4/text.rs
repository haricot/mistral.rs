...
fn q8_tied_output_projection(
    ...
    if !embedding_weight
        .dim(D::Minus1)?
        .is_multiple_of(dtype.block_size())
    {
        return Ok(None);
    }

    // Quantize on CPU to avoid transient BF16/F32 copies of the tied vocab on small GPUs.
    let cpu_weight = embedding_weight.to_device(&Device::Cpu)?;
    let q_weight_cpu = QTensor::quantize(&cpu_weight, dtype)?;
    let qstorage = QStorage::from_data(
        Cow::Owned(q_weight_cpu.data()?.into_owned()),
        target_device,
        dtype,
    )?;
    let q_weight = Arc::new(QTensor::new(qstorage, cpu_weight.shape().clone())?);
...
impl TextModel {
    pub fn new(
        ...
        let (embed_tokens_per_layer, per_layer_model_projection, per_layer_projection_norm) =
            if ple_dim > 0 {
                ...
                // CPU on non-Metal backends so it does not consume most of small GPUs.
    ...
    fn compute_ple(
        ...
        let embedded = if ple_emb.embeddings().device().is_cpu() && !ple_input_ids.device().is_cpu()
        {
