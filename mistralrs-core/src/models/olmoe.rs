...
impl MoeMlp {
    fn new(
        ...
        if cfg.num_shared_experts > cfg.num_experts {
            candle_core::bail!(
                "num_shared_experts ({}) cannot exceed num_experts ({})",
                cfg.num_shared_experts,
                cfg.num_experts
            );
        }
        if cfg.num_shared_experts > cfg.num_experts_per_tok {
            candle_core::bail!(
                "num_shared_experts ({}) cannot exceed num_experts_per_tok ({})",
                cfg.num_shared_experts,
    ...
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        let (topk_ids, mut topk_weights) = if self.num_shared_experts == 0 {
            ...
            let shared_ids = Tensor::from_vec(
                (0..num_tokens)
                    .flat_map(|_| {
                        (self.num_routed_experts..self.num_experts).map(|expert| expert as u32)
                    })
                    .collect::<Vec<_>>(),
            ...
            if routed_k == 0 {
                ...
                let routed_ids = routed_weights
                    .arg_sort_last_dim(false)?
                    .narrow(D::Minus1, 0, routed_k)?
                    .contiguous()?;
                let routed_topk_weights = routed_weights.gather(&routed_ids, D::Minus1)?;
                (
...
struct DecoderLayer {
    self_attn: Attention,
    mlp: MoeMlp,
    pre_attention_layernorm: RmsNorm,
...
impl DecoderLayer {
    ...
    fn new(
        ...
        let mlp = MoeMlp::new(cfg, mlp_vb, layer_device, comm, loading_isq)?;
        let pre_attention_layernorm = RmsNorm::new(
            cfg.hidden_size,
            cfg.rms_norm_eps,
            mapper.set_device(layer_idx, vb.pp("pre_attention_layernorm"), false),
        )?;
...
impl Model {
    ...
    pub fn new_inner(
        ...
        if std::env::var_os("MISTRALRS_OLMOE_ALLOW_F16").is_none()
            && matches!(cfg.dtype.as_deref(), Some("float32" | "float"))
            && vb_m.dtype() == DType::F16
        {
            candle_core::bail!(
...
impl IsqModel for Model {
    ...
    fn residual_tensors(&self) -> Vec<(String, Tensor)> {
        ...
        for (layer_idx, layer) in self.layers.iter().enumerate() {
            ...
            uvb_l
                .pp("pre_attention_layernorm")
                .add(&layer.pre_attention_layernorm);
            uvb_l
                .pp("pre_feedforward_layernorm")
                .add(&layer.pre_feedforward_layernorm);
            uvb_l.pp("mlp").pp("gate").add(layer.mlp.gate());
        }
