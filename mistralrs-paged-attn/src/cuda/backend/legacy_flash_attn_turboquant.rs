...
fn legacy_flash_attn_decode_turboquant_head512_twopass_t<T: CudaDType + DeviceRepr>(
    query: &Tensor,
    key_cache: &Tensor,
    value_cache: &Tensor,
    block_tables: &Tensor,
    ...
    if max_seq_len == 0 {
        candle_core::bail!(
            "legacy_flash_attn_decode_turboquant_head512_twopass expects max_seq_len > 0"
        );
    }
    if key_cache.dtype() != DType::U8 || value_cache.dtype() != DType::U8 {
        candle_core::bail!(
            "legacy_flash_attn_decode_turboquant_head512_twopass expects u8 TurboQuant cache tensors, got {:?} and {:?}",
            key_cache.dtype(),
            value_cache.dtype()
        );
    }
    if !matches!(block_tables.dtype(), DType::I32 | DType::U32) {
        candle_core::bail!(
            "legacy_flash_attn_decode_turboquant_head512_twopass expects i32/u32 block_tables, got {:?}",
            block_tables.dtype()
        );
    }
    if !matches!(cu_seq_lens.dtype(), DType::I32 | DType::U32) {
        candle_core::bail!(
            "legacy_flash_attn_decode_turboquant_head512_twopass expects i32/u32 cu_seq_lens, got {:?}",
            cu_seq_lens.dtype()
        );
    }
    ...
    let (num_seqs, num_heads, q_len, head_dim) = match q_rank {
        ...
        4 => query.dims4()?,
        other => candle_core::bail!(
            "legacy_flash_attn_decode_turboquant_head512_twopass expects query rank 3 or 4, got rank {other} shape {:?}",
            query.shape()
        ),
    };
    if q_len != 1 {
        candle_core::bail!(
            "legacy_flash_attn_decode_turboquant_head512_twopass expects q_len == 1, got {q_len}"
        );
    }
    if head_dim != 512 {
        candle_core::bail!("legacy_flash_attn_decode_turboquant_head512_twopass expects head_dim == 512, got {head_dim}");
    }
    ...
    if (v_num_blocks, v_num_kv_heads, v_block_size) != (num_blocks, num_kv_heads, block_size) {
        candle_core::bail!(
            "legacy_flash_attn_decode_turboquant_head512_twopass cache layout mismatch: key={:?}, value={:?}",
            key_cache.shape(),
            value_cache.shape()
        );
    }
    ...
    let k_head_dim = head_dim_from_row_bytes(k_row_bytes)?;
    let v_head_dim = head_dim_from_row_bytes(v_row_bytes)?;
    if k_head_dim != 512 || v_head_dim != 512 {
        candle_core::bail!(
            "legacy_flash_attn_decode_turboquant_head512_twopass head dim mismatch: q=512, k={k_head_dim}, v={v_head_dim}"
        );
    }
    if num_heads % num_kv_heads != 0 {
        candle_core::bail!(
            "legacy_flash_attn_decode_turboquant_head512_twopass: num_heads ({num_heads}) must be divisible by num_kv_heads ({num_kv_heads})"
        );
    }
    ...
    if bt_num_seqs != num_seqs {
        candle_core::bail!(
            "legacy_flash_attn_decode_turboquant_head512_twopass block_tables seq mismatch: got {bt_num_seqs}, expected {num_seqs}"
        );
    }
    let cu_seq_lens_len = cu_seq_lens.dims1()?;
    if cu_seq_lens_len != num_seqs + 1 {
        candle_core::bail!(
            "legacy_flash_attn_decode_turboquant_head512_twopass cu_seq_lens length mismatch: got {cu_seq_lens_len}, expected {}",
            num_seqs + 1
        );
    }
    ...
    let out = if q_rank == 4 {
        Tensor::zeros(
            (num_seqs, num_heads, 1usize, 512usize),
            query.dtype(),
            query.device(),
        )?
    } else {
        Tensor::zeros(
            (num_seqs, num_heads, 512usize),
            query.dtype(),
            query.device(),
        )?
    };
    let scores = Tensor::zeros(
        (num_seqs, num_heads, max_seq_len),
        DType::F32,
        query.device(),
    )?;
    let dtype_code = dtype_code(query.dtype())?;
    ...
    let q_s = match &*q_s {
        Storage::Cuda(s) => s,
        _ => candle_core::bail!(
            "legacy_flash_attn_decode_turboquant_head512_twopass: query must be CUDA"
        ),
    };
    let (kc_s, kc_l) = key_cache.storage_and_layout();
    let kc_s = match &*kc_s {
        Storage::Cuda(s) => s,
        _ => candle_core::bail!(
            "legacy_flash_attn_decode_turboquant_head512_twopass: key_cache must be CUDA"
        ),
    };
    let (vc_s, vc_l) = value_cache.storage_and_layout();
    let vc_s = match &*vc_s {
        Storage::Cuda(s) => s,
        _ => candle_core::bail!(
            "legacy_flash_attn_decode_turboquant_head512_twopass: value_cache must be CUDA"
        ),
    };
    ...
    let o_s = match &*o_s {
        Storage::Cuda(s) => s,
        _ => candle_core::bail!(
            "legacy_flash_attn_decode_turboquant_head512_twopass: output must be CUDA"
        ),
    };
    let scores_binding = scores.clone();
    let (sc_s, sc_l) = scores_binding.storage_and_layout();
    let sc_s = match &*sc_s {
        Storage::Cuda(s) => s,
        _ => candle_core::bail!(
            "legacy_flash_attn_decode_turboquant_head512_twopass: scores must be CUDA"
        ),
    };
    ...
    let (vc_ptr, _vc_guard) = slice_ptr(vc_s.as_cuda_slice::<u8>()?, vc_l.start_offset());
    let (out_ptr, _out_guard) = slice_ptr(o_s.as_cuda_slice::<T>()?, o_l.start_offset());
    let (scores_ptr, _scores_guard) = slice_ptr(sc_s.as_cuda_slice::<f32>()?, sc_l.start_offset());

    let (bt_s, bt_l) = block_tables.storage_and_layout();
    let bt_s = match &*bt_s {
        Storage::Cuda(s) => s,
        _ => candle_core::bail!(
            "legacy_flash_attn_decode_turboquant_head512_twopass: block_tables must be CUDA"
        ),
    };
    let (bt_ptr, _bt_guard) = if block_tables.dtype() == DType::I32 {
        let (ptr, guard) = slice_ptr(bt_s.as_cuda_slice::<i32>()?, bt_l.start_offset());
    ...
    let cu_s = match &*cu_s {
        Storage::Cuda(s) => s,
        _ => candle_core::bail!(
            "legacy_flash_attn_decode_turboquant_head512_twopass: cu_seq_lens must be CUDA"
        ),
    };
    let (cu_ptr, _cu_guard) = if cu_seq_lens.dtype() == DType::I32 {
        let (ptr, guard) = slice_ptr(cu_s.as_cuda_slice::<i32>()?, cu_l.start_offset());
    ...
    let dev = q_s.device();
    unsafe {
        ffi::legacy_flash_attn_decode_turboquant_head512_twopass(
            q_ptr as *const core::ffi::c_void,
            kc_ptr as *const core::ffi::c_void,
            vc_ptr as *const core::ffi::c_void,
            bt_ptr,
...
/// Specialized two-pass TurboQuant-direct decode kernel for head_dim=512.
///
/// This path materializes only f32 attention scores `[num_seqs, num_heads, max_seq_len]`,
/// not dense K/V. It uses 256 CUDA threads per head/sequence instead of one warp, reducing
/// FWHT latency and register pressure for Gemma/OmniCoder-style `D=512` heads on SM61.
#[allow(clippy::too_many_arguments)]
pub fn legacy_flash_attn_decode_turboquant_head512_twopass(
    ...
    match query.dtype() {
        DType::F16 => legacy_flash_attn_decode_turboquant_head512_twopass_t::<half::f16>(
            query,
            key_cache,
            value_cache,
            block_tables,
        ...
        DType::BF16 => legacy_flash_attn_decode_turboquant_head512_twopass_t::<half::bf16>(
            query,
            key_cache,
            value_cache,
            block_tables,
        ...
        DType::F32 => legacy_flash_attn_decode_turboquant_head512_twopass_t::<f32>(
            query,
            key_cache,
            value_cache,
            block_tables,
        ...
        dt => candle_core::bail!(
            "legacy_flash_attn_decode_turboquant_head512_twopass unsupported dtype {dt:?}"
        ),
    }
}
